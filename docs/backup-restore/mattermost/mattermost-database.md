# Mattermost Database Backup and Restore

Mattermost stores data in a postgres database. We host this database in AWS RDS.
...
The database is definitely the easiest piece of mattermost to restore.
...
- Modify the terraform vars for the new environment with the following variable, replacing `<snapshot-name>` with the snapshot name you want to restore from:
  ...
  ```hcl
  mattermost_db_snapshot = "<snapshot-name>"
  ```
